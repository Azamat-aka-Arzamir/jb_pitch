class SoftBodyCreator {
	...
	static CreateCube(inGridSize = 5, inGridSpacing = 0.5, edgeCompliance = 0, volumeCompliance = 0,wf=2,hf=.6,lf=1.4) {
		...
		for (let z = 0; z < inGridSize; ++z)
			for (let y = 0; y < inGridSize; ++y)
				for (let x = 0; x < inGridSize; ++x) {
					let f = +Math.pow((x-inGridSize/2+.5)*1.4,2)*(20-Math.pow((x-inGridSize/2+.5)*.6,2))*.001*y/inGridSize+Math.pow((z-inGridSize/2+.5)*1.4,2)*(20-Math.pow((z-inGridSize/2+.5)*.6,2))*.0002*y/inGridSize;
					v.mPosition.x = inGridSpacing*wf * x + cOffset*wf;
					v.mPosition.y = inGridSpacing*hf * y + cOffset*hf+f*.5;
					v.mPosition.z = inGridSpacing*lf * z + cOffset*lf;
					sharedSettings.mVertices.push_back(v);
				}
		Jolt.destroy(v);
